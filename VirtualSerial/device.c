...
--*/
{
	...
	DECLARE_UNICODE_STRING_SIZE  (ipAddr, 20);
    ...
    // query IPAddr from subkey
    ULONG tempIPAddr = { 0 };
    status = WdfRegistryQueryULong(
        subkey, 
        &ipAddrName, 
		&tempIPAddr);
    // if not find valuename, retrieve default value from hkey and assign it to subkey
	if (status == STATUS_NO_SUCH_FILE) {
		...
		if (!NT_SUCCESS(status)) {
			Trace(TRACE_LEVEL_ERROR,
				"Error: Failed to read ipAddrName");
			goto Exit;
		}
        ...
        if (!NT_SUCCESS(status)) {
            Trace(TRACE_LEVEL_ERROR,
                "Error: Failed to assign ipAddrNameS");
            goto Exit;
        }
	} else if (!NT_SUCCESS(status)) {
		Trace(TRACE_LEVEL_ERROR,
			"Error: Failed to Assign IPAddr a new value error code: %x", status);
		goto Exit;
	}

    // query portNum from subkey
    ULONG tempPortNum = { 0 };
	status = WdfRegistryQueryULong(
        subkey,
		&portNumName,
	...
	if (status == STATUS_NO_SUCH_FILE) {
		...
		Trace(TRACE_LEVEL_ERROR,
			"Error: Failed to read PortNum, error code: %x", status);
		goto Exit;
	}

    // Assign IP and PortNum to Device Context
